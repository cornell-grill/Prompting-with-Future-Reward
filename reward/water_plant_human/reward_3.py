...
def should_release(context, stage, env_id=0):
    ...
    tilt_angle = get_sideways_tilt_deg(cup, env_id=env_id)
    return bool(cup_pos[2] > plant_top_z and 90.0 <= tilt_angle <= 180.0)
...
def compute_reward(context, prev_context, stage, env_id=0):
    ...
    tilt_angle = get_sideways_tilt_deg(cup, env_id=env_id)
    reward += 10.0 * min(tilt_angle, 30.0) / 30.0
    if 90.0 <= tilt_angle <= 180.0:
        reward += 20.0 * ((tilt_angle - 90.0) / 90.0)
